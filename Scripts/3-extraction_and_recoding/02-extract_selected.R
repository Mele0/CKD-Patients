...
extraction_recoding_dir <- as.character(args[2])
output_dir <- as.character(args[3])

# Creating directory for codings
dir.create(file.path(extraction_recoding_dir, "parameters/codings"), showWarnings = FALSE)

# Loading xlsx file with selections
choices <- read.xlsx(file.path(extraction_recoding_dir, "parameters/selection.xlsx"))
...
codings <- fread(file.path(extraction_recoding_dir, "docs/Codings.csv"), data.table = FALSE) # Codings for categorical outcomes: https://biobank.ctsu.ox.ac.uk/crystal/exinfo.cgi?src=accessing_data_guide
...
for (i in 1:length(coding_ids)) {
  ...
  codes_filepath <- file.path(extraction_recoding_dir, paste0("parameters/codings/codes_", codeID, ".txt"))
  if (!file.exists(codes_filepath)) {
    print(paste0("CREATING NEW CODINGS: codes_", codeID, ".txt"))
    tmp <- codings[codings$Coding %in% codeID, -1]
    tmp <- cbind(tmp, tmp)
    colnames(tmp) <- c("OriginalValue", "OriginalMeaning", "RecodedValue", "RecodedMeaning")
    write.table(tmp, codes_filepath,
      row.names = FALSE, col.names = TRUE
    )
  } 
  # else {
  #   print(paste0("Leaving existing codes_", codeID, ".txt unchanged"))
  # }
...
write.table(tmp, file.path(extraction_recoding_dir, paste0("parameters/codings/codes_template_continuous.txt")),
  row.names = FALSE, col.names = TRUE
)
...
saveRDS(choices, file.path(output_dir, "annot.rds"))

# Saving extracted dataset
saveRDS(mydata, file.path(output_dir, "ukb_extracted.rds"))
