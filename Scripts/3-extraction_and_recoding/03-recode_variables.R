...
extraction_recoding_dir <- as.character(args[1])
output_dir <- as.character(args[2])

# Loading outputs from previous steps
choices <- readRDS(file.path(output_dir, "annot.rds"))
mydata <- readRDS(file.path(output_dir, "ukb_extracted.rds"))
...
for (k in 1:ncol(mydata)) {
  ...
  if (!is.na(tmp_coding_id)) {
    ...
    coding <- read.table(file.path(extraction_recoding_dir, paste0("parameters/codings/codes_", tmp_coding_id, ".txt")), header = TRUE, stringsAsFactors = FALSE)
  ...
  if (grepl("date", tolower(choices[tmp_coding_name, "ValueType"]))) {
    # Recoding for dates
    print(paste("SKIP RECODING DATE VAR:", tmp_coding_name))
...
continuous_codings <- list.files(path = file.path(extraction_recoding_dir, "parameters/codings"), pattern = "codes_field")
if (length(continuous_codings) > 0) {
  ...
  for (i in length(fields_to_recode)) {
    ...
    coding <- read.table(file.path(extraction_recoding_dir, paste0("parameters/codings/", continuous_codings[i])),
...
write.xlsx(choices, file.path(extraction_recoding_dir, "parameters/parameters.xlsx"))

# Saving extracted dataset
saveRDS(mydata, file.path(output_dir, "ukb_recoded.rds"))
