...
# Logistic regression
covars_train_demographics$cluster_bin <- ifelse(covars_train_demographics$cluster == 1, 1, 0)
...
plot(selprop, type = "h", lwd = 3, las = 1, xlab = "",
     ...
     main = "Stability-Lasso selected demographics variables for Cluster 1 vs 0/2",
...
selection_results <- cbind(cluster =1, selection_results)
...
# Logistic regression
covars_train_clinical$cluster_bin <- ifelse(covars_train_clinical$cluster == 1, 1, 0)
...
plot(selprop, type = "h", lwd = 3, las = 1, xlab = "",
     ylab = "Selection Proportion", xaxt = "n", bty = "l",
     main = "Stability-Lasso selected clinical variables for Cluster 1 vs 0/2",
     col = ifelse(selprop >= hat_params[2], yes = "red", no = "grey"),
     cex.lab = 1, cex.main = 0.9, ylim = c(0, 1.2))
...
selection_results$eGFR <- 1 # add eGFR which was removed



################# Comorbidities sub-category 
# Logistic regression
covars_train_comorbidities$cluster_bin <- ifelse(covars_train_comorbidities$cluster == 1, 1, 0)
...
plot(selprop, type = "h", lwd = 3, las = 1, xlab = "",
     ylab = "Selection Proportion", xaxt = "n", bty = "l",
     main = "Stability-Lasso selected comorbidities variables for Cluster 1 vs 0/2",
     col = ifelse(selprop >= hat_params[2], yes = "red", no = "grey"),
     cex.lab = 1, cex.main = 0.9, ylim = c(0, 1.2))
...
# Logistic regression
covars_train_labs$cluster_bin <- ifelse(covars_train_labs$cluster == 1, 1, 0)
...
plot(selprop, type = "h", lwd = 3, las = 1, xlab = "",
     ylab = "Selection Proportion", xaxt = "n", bty = "l",
     main = "Stability-Lasso selected lab variables for Cluster 1 vs 0/2",
     col = ifelse(selprop >= hat_params[2], yes = "red", no = "grey"),
     cex.lab = 1, cex.main = 0.9, ylim = c(0, 1.2))
...
# Save as rds file
saveRDS(selection_results, file = "cluster1_selection_results.rds")
...
covars_refit$cluster_bin <- ifelse(covars_refit$cluster == 1, 1, 0)
covars_refit <- covars_refit[, !colnames(covars_refit) %in% "CKDstage_recruit"] # remove CKDstage
covars_test$cluster_bin <- ifelse(covars_test$cluster == 1, 1, 0)
...
# Plot the ROC curve
plot(roc.obj, col = "blue", main = "Cluster 1 ROC")
