...
# Logistic regression
covars_train_demographics$cluster_bin <- ifelse(covars_train_demographics$cluster == 2, 1, 0)
...
plot(selprop, type = "h", lwd = 3, las = 1, xlab = "",
     ...
     main = "Stability-Lasso selected demographics variables for Cluster 2 vs 0/1",
...
selection_results <- cbind(cluster = 2, selection_results)

################# Clinical sub-category 
# Logistic regression
covars_train_clinical$cluster_bin <- ifelse(covars_train_clinical$cluster == 2, 1, 0)
...
plot(selprop, type = "h", lwd = 3, las = 1, xlab = "",
     ylab = "Selection Proportion", xaxt = "n", bty = "l",
     main = "Stability-Lasso selected clinical variables for Cluster 2 vs 0/1",
     col = ifelse(selprop >= hat_params[2], yes = "red", no = "grey"),
     cex.lab = 1, cex.main = 0.9, ylim = c(0, 1.2))
...
# Logistic regression
covars_train_comorbidities$cluster_bin <- ifelse(covars_train_comorbidities$cluster == 2, 1, 0)
...
plot(selprop, type = "h", lwd = 3, las = 1, xlab = "",
     ylab = "Selection Proportion", xaxt = "n", bty = "l",
     main = "Stability-Lasso selected comorbidities variables for Cluster 2 vs 0/1",
     col = ifelse(selprop >= hat_params[2], yes = "red", no = "grey"),
     cex.lab = 1, cex.main = 0.9, ylim = c(0, 1.2))
...
################# Labs sub-category 
# Remove platelet
covars_train_labs <- covars_train_labs[, !colnames(covars_train_labs) %in% "platelet"]

# Logistic regression
covars_train_labs$cluster_bin <- ifelse(covars_train_labs$cluster == 2, 1, 0)
...
plot(selprop, type = "h", lwd = 3, las = 1, xlab = "",
     ylab = "Selection Proportion", xaxt = "n", bty = "l",
     main = "Stability-Lasso selected lab variables for Cluster 2 vs 0/1",
     col = ifelse(selprop >= hat_params[2], yes = "red", no = "grey"),
     cex.lab = 1, cex.main = 0.9, ylim = c(0, 1.2))
...
selection_results$platelet <- 1 # add platelet which was removed

# Save as rds file
saveRDS(selection_results, file = "cluster2_selection_results.rds")

### Load refit and test datasets
covars_refit <- readRDS("covars_refit.rds")
covars_test <- readRDS("covars_test.rds")

covars_refit$cluster_bin <- ifelse(covars_refit$cluster == 2, 1, 0)
covars_refit <- covars_refit[, !colnames(covars_refit) %in% "CKDstage_recruit"] # remove CKDstage
covars_test$cluster_bin <- ifelse(covars_test$cluster == 2, 1, 0)
...
# Plot the ROC curve
plot(roc.obj, col = "blue", main = "Cluster 2 ROC")
